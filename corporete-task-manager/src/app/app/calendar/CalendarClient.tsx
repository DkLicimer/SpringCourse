...
export function CalendarClient({ initialEvents, isAdmin, currentUserId, users = [] }: CalendarClientProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const input = {
      ...
      participantIds: selectedParticipants, // <-- Передаем выбранных участников в Action
    };
              ...
              .map((event) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Вывод участников в мобильной карточке */}
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl border border-slate-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              {/* ⚡ Множественный выбор сотрудников для совещания (ВСТРЕЧА ДЛЯ ШТАТА) */}
              {(type === "FREE" || type === "GC") && users.length > 0 && ( // <-- ИСПРАВЛЕНО: Даем выбор сотрудников и для "Свободно" (FREE) и для "Главного корпуса" (GC) по требованию заказчика!
