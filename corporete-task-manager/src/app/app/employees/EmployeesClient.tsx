...
export function EmployeesClient({ initialUsers, currentUserId }: EmployeesClientProps) {
  ...
  const [access, setAccess] = useState({
    ...
    canReadPost: false,
    canWritePost: false,
    canReadInfo: false,  // <-- НОВОЕ: Право чтения ИНФОпространства
    canWriteInfo: false, // <-- НОВОЕ: Право записи ИНФОпространства
  });
  ...
  const openEditModal = (user: UserWithAccess) => {
    ...
    const info = user.tableAccesses.find(a => a.tableName === "info_space"); // <-- Находим доступы ИНФОпространства
    ...
    setAccess({
      ...
      canReadInfo: info?.canRead || false,   // <-- Восстанавливаем
      canWriteInfo: info?.canWrite || false, // <-- Восстанавливаем
    });
    setIsOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Передаем доступы ИНФОпространства в Action
        ...
        <table className="min-w-full divide-y divide-slate-200 text-xs">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left font-semibold text-slate-500 uppercase tracking-wider">ИНФОпространство</th> {/* <-- НОВАЯ КОЛОНКА */}
          ...
          <tbody className="divide-y divide-slate-200 bg-white">
            {filteredUsers.map((user) => {
              ...
              const infoAccess = user.tableAccesses.find((a) => a.tableName === "info_space"); // <-- Считываем доступ ИНФОпространства
              ...
              return (
                <tr key={user.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  {/* ⚡ НОВОЕ: Отображение прав на ИНФОпространство в общей таблице */}
                  <td className="px-6 py-4 whitespace-nowrap">
      ...
      <div className="block lg:hidden space-y-4">
        {filteredUsers.length === 0 ? (
          ...
          filteredUsers.map((user) => {
            ...
            const contentAccess = user.tableAccesses.find((a) => a.tableName === "content_plan");
            const postAccess = user.tableAccesses.find((a) => a.tableName === "post_request");
            const infoAccess = user.tableAccesses.find((a) => a.tableName === "info_space"); // <-- Считываем
            ...
            return (
              <div key={user.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  {/* ⚡ НОВОЕ: Отображение прав на ИНФОпространство в мобильной карточке */}
                  <div className="bg-slate-50 p-2 rounded-lg border border-slate-100 flex justify-between items-center">
                    <span className="text-slate-400 font-medium text-[10px]">ИНФОпространство</span>
                    {user.role === "ADMIN" ? <span className="text-[10px] font-bold text-slate-400">ПОЛНЫЙ</span> : renderAccessBadge(infoAccess?.canRead || false, infoAccess?.canWrite || false)}
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl border border-slate-200 flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4" autoComplete="off">
              ...
              <div className="space-y-3">
                ...
                {/* Текстовое поле ввода подразделения/отдела */}
                <div>
