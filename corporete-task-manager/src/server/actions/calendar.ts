...
interface CreateEventInput {
  ...
  type: "FREE" | "GC" | "BUSY";
  description?: string;
  participantIds?: string[]; // IDs приглашенных сотрудников
}
...
export async function createCalendarEvent(input: CreateEventInput) {
  ...
  // ⚡ УМНАЯ ЗАЩИТА: Наложение проверяется только на блокировки BUSY (красные)
  // ИЛИ на встречи, уже забронированные другими сотрудниками.
  // Пересекать свободные окна администратора (FREE и GC) РАЗРЕШЕНО.
  const overlappingEvent = await prisma.calendarEvent.findFirst({
    where: {
      startTime: { lt: end },
      endTime: { gt: start },
      OR: [
        { type: "BUSY" }, // Блокирует любое наложение
        {
          bookedBy: {
            role: "EMPLOYEE" // Блокирует наложение на существующие брони сотрудников
          }
        }
      ]
    },
  ...
  // Автоматическая рассылка писем и уведомлений приглашенным (ВСТРЕЧА ДЛЯ ШТАТА)
