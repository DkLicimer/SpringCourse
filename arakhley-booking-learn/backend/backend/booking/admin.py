...
# РЕГИСТРАЦИЯ ГАЛЕРЕИ В АДМИНКЕ С ИНТЕРАКТИВНЫМ ПРЕВЬЮ
@admin.register(GalleryPhoto)
class GalleryPhotoAdmin(admin.ModelAdmin):
    list_display = ['image_preview', 'caption', 'is_active', 'uploaded_at']
    ...
    def image_preview(self, obj):
        if obj.image:
            return format_html(
                '<img src="{0}" style="max-height: 48px; border-radius: 4px; border: 1px solid #ccc; padding: 1px;" />',
                obj.image.url
            )
    ...
    def image_preview_large(self, obj):
        if obj.image:
            return format_html(
                '<img src="{0}" style="max-height: 240px; border-radius: 8px; border: 1px solid #ddd; box-shadow: 0 2px 8px rgba(0,0,0,0.1);" />',
                obj.image.url
            )
        return "Нет изображения"
    image_preview_large.short_description = "Крупный просмотр"