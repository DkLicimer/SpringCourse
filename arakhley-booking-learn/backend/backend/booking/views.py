...
from .models import Cabin, Booking, PaymentReceipt, GalleryPhoto
from .serializers import CabinSerializer, BookingCreateSerializer, ReceiptUploadSerializer, GalleryPhotoSerializer
    ...
    def get(self, request):
        ...
        for b in active_bookings:
            ...
            if b.second_cabin_id and b.second_cabin_id in cabin_occupancy:
                cabin_occupancy[b.second_cabin_id] += b.second_cabin.capacity
        ...
        for cabin in Cabin.objects.all():
            ...
            if cabin.is_active:
                if role == 'STAFF':
                    ...
                    # Студенты видят домик только если для них установлена цена больше 0
                    if cabin.price_student_bed > 0:
                        is_available = (booked_count + beds_requested <= cabin.capacity)
...
class BookingCancelView(APIView):
    def post(self, request, booking_id):
        ...
        if booking.status != Booking.Status.PENDING_PAYMENT:
            return Response({"error": "Это бронирование уже нельзя отменить автоматически."}, status=400)
            
        booking.status = Booking.Status.CANCELLED
        booking.save()
        send_booking_notification(booking, "USER_CANCELLED")
        return Response({"message": "Бронирование успешно отменено."}, status=200)
