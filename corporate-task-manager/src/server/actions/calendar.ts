...
// Форматирование даты под часовой пояс Читы (UTC+9)
function formatChitaTime(date: Date): string {
...
export async function createCalendarEvent(input: CreateEventInput) {
  ...
  const chitaFormattedTime = formatChitaTime(start);
    ...
    try {
      ...
      for (const admin of admins) {
        await createNotification(
          admin.id,
          `Сотрудник ${session.user.name} записался на встречу "${title}" на ${chitaFormattedTime} (Чита).`,
          `/app/calendar`
        );
      }
    } catch (err) {
  ...
  if (participantIds && participantIds.length > 0) {
    try {
      ...
      for (const user of invitedUsers) {
        await createNotification(
          user.id,
          `Приглашение на встречу «${title}» на ${chitaFormattedTime} (Чита).`,
          `/app/calendar`
        );
          ...
          const emailHtml = `
            <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 600px; border: 1px solid #e2e8f0; border-radius: 12px;">
              ...
              <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 14px;">
                ...
                <tr>
                  <td style="padding: 8px 0; border-bottom: 1px solid #f1f5f9; color: #64748b;">Дата и время (Чита, UTC+9):</td>
                  <td style="padding: 8px 0; border-bottom: 1px solid #f1f5f9; font-weight: bold; color: #1e293b;">${chitaFormattedTime}</td>
                </tr>
              </table>
