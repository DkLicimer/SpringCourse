...
export function UrgentTasksWidget() {
  ...
  return (
    <>
      {/* 1. МОБИЛЬНАЯ ВЕРСИЯ (Круглая кнопка + Всплывающее модальное окно) */}
      <div className="md:hidden">
        {/* Круглая плавающая кнопка */}
        <button
          onClick={() => setIsOpen(true)}
          className="fixed bottom-6 right-6 z-40 h-14 w-14 rounded-full bg-red-600 hover:bg-red-700 text-white shadow-2xl flex items-center justify-center transition-all animate-pulse border border-red-500 cursor-pointer"
        >
          <AlertCircle className="h-6 w-6" />
          <span className="absolute -top-1 -right-1 h-6 w-6 rounded-full bg-slate-900 border-2 border-white text-[10px] font-black flex items-center justify-center text-white">
            {urgentTasks.length}
          </span>
        </button>
        ...
        {isOpen && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
              <div className="px-4 py-3.5 bg-red-600 text-white flex justify-between items-center shrink-0">
              ...
              <div className="p-4 space-y-3 divide-y divide-slate-100 overflow-y-auto">
                {urgentTasks.map((task, idx) => (
                  <div 
                    key={task.id} 
                    onClick={() => { router.push("/app/tasks"); setIsOpen(false); }}
                  ...
                  >
                    ...
                    <h5 className="font-bold text-slate-800 leading-snug">{task.title}</h5>
                    {task.deadline && (
      ...
      {/* 2. ДЕСКТОПНАЯ ВЕРСИЯ (Классическая панель) */}
      <div className="hidden md:block fixed bottom-6 right-6 z-40 max-w-sm w-80 bg-white rounded-2xl border border-slate-200 shadow-2xl overflow-hidden print:hidden">
        <div 
