...
export function GeneralChatWidget({ currentUserId, isAdmin }: GeneralChatWidgetProps) {
  ...
  const calculateUnread = useCallback((msgs: ChatMessage[]) => {
  ...
  }, [isOpen, markAsRead, calculateUnread]);

  // ⚡ УМНЫЙ ПОЛЛИНГ: когда чат закрыт — опрашиваем раз в 15 секунд (экономия CPU сервера),
  // когда чат открыт — опрашиваем раз в 4 секунды для мгновенной переписки
  useEffect(() => {
    fetchMessages();
    const pollInterval = isOpen ? 4000 : 15000;
