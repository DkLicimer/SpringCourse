...
export function PostRequestClient({ initialRequests, canManage }: PostRequestClientProps) {
  ...
  // Стейт для отслеживания развернутых описаний заявок
  const [expandedRequestIds, setExpandedRequestIds] = useState<string[]>([]);

  const toggleExpandRequest = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ========================================================================= */}
      {/* 💻 ДЕСКТОПНАЯ ВЕРСИЯ ТАБЛИЦЫ С УЛУЧШЕННОЙ ПРОКРУТКОЙ И РАСКРЫТИЕМ */}
      {/* ========================================================================= */}
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider min-w-[300px]">Тема и Описание</th>
                ...
                return (
                  <tr key={req.id} className="hover:bg-slate-50/50 transition-colors text-sm">
                    ...
                    <td className="px-6 py-4">
                      ...
                      {/* Интерактивное сворачивание/разворачивание описания */}
                      <div className="space-y-1 mt-1">
                        ...
                        {req.description && req.description.length > 100 && (
                          <button
                            onClick={(e) => toggleExpandRequest(req.id, e)}
                            className="text-[10px] text-blue-600 font-bold hover:underline flex items-center gap-0.5 focus:outline-none cursor-pointer"
                          >
                            {isExpanded ? (
                              <>Свернуть <ChevronUp className="h-3 w-3" /></>
                            ) : (
                              <>Развернуть полностью <ChevronDown className="h-3 w-3" /></>
                            )}
                          </button>
                        )}
                      </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    required
                    placeholder="Опишите суть работы, прикрепите ссылки на картинки..."
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-blue-500 text-slate-800"
                  />
                </div>
              </div>
