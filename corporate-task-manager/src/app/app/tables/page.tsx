...
export default async function TablesHubPage() {
  ...
  // ⚡ НОВОЕ: Проверяем доступы для ИНФОпространства
  const canReadInfo = isAdmin || userAccesses.find(a => a.tableName === "info_space")?.canRead || false;
  ...
  const tables = [
    ...
    },
    {
      ...
      hasAccess: canReadInfo, // <-- ИЗМЕНЕНО: Заменяем захардкоженный true на реальные права
    },
  ];
