...
import { 
  ...
  updateGoal,
  deleteGoal,
  addComment // Импортируем новый экспорт
...
type Task = {
  ...
  comments: Comment[]; // Добавляем комментарии в тип задачи
};
...
export function TasksClient({
  ...
  // Специфические стейты для создания задачи
  ...
  const handleSendComment = async (e: React.FormEvent) => {
    ...
    setCommentText(""); // Очищаем поле ввода сразу

    // 1. Создаем "оптимистичный" временный комментарий для мгновенного показа на экране
    ...
    // 2. Мгновенно обновляем стейт активной задачи, чтобы пользователь сразу увидел сообщение
    if (activeTask) {
      setActiveTask({
        ...activeTask,
    ...
    startTransition(async () => {
      try {
        // 3. В фоне отправляем реальный запрос на сервер
        await addComment(activeTask!.id, tempCommentText);
        
        // 4. После успеха подтягиваем актуальный массив задач с сервера (где у сообщения уже будет реальный ID)
        ...
        // Если произошла ошибка — откатываем чат к исходному состоянию
  ...
  return (
    <div className="space-y-6">
        ...
        {isAdmin && (
          <div className="flex flex-wrap gap-2 w-full md:w-auto">
            <button
              ...
              className="flex items-center justify-center gap-2 border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 px-3.5 py-2 rounded-lg text-sm font-semibold transition-colors"
      ...
      {/* МОДАЛКА: УПРАВЛЕНИЕ ЦЕЛЯМИ (ADMIN ONLY) */}
      {isManageGoalsOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      ...
      {isTaskOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl border border-slate-200 flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-200">
              <h3 className="text-lg font-bold text-slate-800">Добавить новую задачу</h3>
      ...
      {/* МОДАЛКА: ПРОСМОТР ДЕТАЛЕЙ ЗАДАЧИ С МИНИ-ЧАТОМ */}
      {activeTask && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl border border-slate-200 flex flex-col h-[90vh]">
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-200">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div>
                ...
                {activeTask.description && (
                  <p className="text-sm text-slate-600 mt-2 whitespace-pre-line bg-slate-50 p-3 rounded-lg border">
                    {activeTask.description}
                  </p>
                )}
              </div>
              ...
              {/* ========================================================================= */}
              {/* ФИНАЛЬНЫЙ БЛОК: ОБСУЖДЕНИЕ (МИНИ-ЧАТ) */}
              {/* ========================================================================= */}
              <div className="border-t border-slate-200 pt-4 space-y-3">
                ...
                <form onSubmit={handleSendComment} className="flex gap-2 items-center">
                  ...
                  <button
                    ...
                    className="p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors disabled:bg-blue-400 shrink-0"
