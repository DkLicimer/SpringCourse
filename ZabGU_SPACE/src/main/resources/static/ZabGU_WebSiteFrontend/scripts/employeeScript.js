document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- НОВАЯ ЛОГИКА НАВИГАЦИИ ПО РАЗДЕЛАМ ---
    adminNavButtons.forEach(button => {
        button.addEventListener('click', () => {
            const targetSectionId = `section-${button.dataset.section}`;

            // Убираем класс 'active' со всех кнопок и секций
            adminNavButtons.forEach(btn => btn.classList.remove('active'));
            adminSections.forEach(section => section.classList.remove('active'));

            // Добавляем класс 'active' только кликнутой кнопке и её секции
    ...
    function renderApplications(applications) {
        ...
        applications.forEach(app => {
            ...
            // Форматируем время для отображения (часы и минуты)
            const startTime = new Date(app.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
            const endTime = new Date(app.endTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
            ...
            item.innerHTML = `
                <div class="item-summary">
                    ...
                    <span class="item-time">${timeString}</span> <!-- ДОБАВЛЕНО ВРЕМЯ -->
